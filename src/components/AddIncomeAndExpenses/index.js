...
const IncomeAndExpensesComponent = () => {
    ...
    const fetchReports = async () => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:8000/api/income-and-expense-reports', {
    ...
    const handleSubmit = async (e) => {
        ...
        const token = localStorage.getItem('token');
        const url = editingId
            ? `http://localhost:8000/api/income-and-expense-reports/${editingId}`
            : 'http://localhost:8000/api/income-and-expense-reports';
    ...
    const handleDelete = async (id) => {
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`http://localhost:8000/api/income-and-expense-reports/${id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (response.ok) {
        ...
        <Container maxWidth="lg">
            <Paper elevation={3} sx={{ p: 4, mt: 4 }}>
                <Typography variant="h4" component="h1" gutterBottom>
                    {editingId ? 'Редактировать отчет' : 'Добавить отчет'}
                </Typography>
...
export default IncomeAndExpensesComponent;
