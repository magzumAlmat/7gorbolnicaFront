...
const FinancialReportsComponent = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const token = localStorage.getItem('token');
        // Assuming the backend expects multipart/form-data due to the 'path' field
        const url = editingId
            ? `http://localhost:8000/api/finance-reports/${editingId}`
            : 'http://localhost:8000/api/finance-reports';
    ...
    const handleDelete = async (id) => {
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`http://localhost:8000/api/finance-reports/${id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (response.ok) {
        ...
        <Container maxWidth="lg">
            <Paper elevation={3} sx={{ p: 4, mt: 4 }}>
                <Typography variant="h4" component="h1" gutterBottom>
                    {editingId ? 'Редактировать финансовый отчет' : 'Добавить финансовый отчет'}
                </Typography>
...
export default FinancialReportsComponent;
