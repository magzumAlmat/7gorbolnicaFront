...
const PricesComponent = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const token = localStorage.getItem('token');
        const url = editingId
            ? `http://localhost:8000/api/prices/${editingId}`
        ...
        try {
            ...
            if (response.ok) {
                ...
                setError(`Failed to save price: ${errorData.message || 'Unknown error'}`);
            }
        } catch (error) {
            setError('Error saving price: ' + error.message);
        }
    };
    ...
    const handleEdit = (price) => {
        ...
        setFile(null);
        if (price.path) {
            setPreview(`http://localhost:8000/${price.path}`);
        } else {
            setPreview(null);
        }
    };
    ...
    const handleDelete = async (id) => {
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`http://localhost:8000/api/prices/${id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (response.ok) {
            ...
            setError('Error deleting price: ' + error.message);
        }
    };
        ...
        <Container maxWidth="lg">
            <Paper elevation={3} sx={{ p: 4, mt: 4 }}>
                <Typography variant="h4" component="h1" gutterBottom>
                    {editingId ? 'Редактировать прайс-лист' : 'Добавить прайс-лист'}
                </Typography>
                    ...
                    <Grid container spacing={3}>
                        ...
                        <Grid item xs={12}>
                            <Button type="submit" fullWidth variant="contained" startIcon={editingId ? <Edit /> : <Add />} sx={{ mt: 2, height: '56px' }}>
                                {editingId ? 'Обновить прайс-лист' : 'Добавить прайс-лист'}
                            </Button>
                        </Grid>
                        {editingId && (
                            <Grid item xs={12}>
                    ...
                    <List>
                        {prices.map((price) => (
                            <ListItem
                                ...
                                secondaryAction={
                                    <>
                                        <IconButton edge="end" aria-label="edit" onClick={() => handleEdit(price)} sx={{ mr: 1 }}>
                                            <Edit />
                                        </IconButton>
                                        <IconButton edge="end" aria-label="delete" onClick={() => handleDelete(price.id)}>
                                            <Delete />
                                        </IconButton>
                                    </>
                                }
                            ...
                            >
                                <ListItemText
                                    primary={`${price.name} (${price.year})`}
                                    secondary={price.path ? (
                                        <Link href={`http://localhost:8000/${price.path}`} target="_blank" rel="noopener">
                                            Скачать файл
                                        </Link>
                                    ) : 'Файл не загружен'}
                                />
