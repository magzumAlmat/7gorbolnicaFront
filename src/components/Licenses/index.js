...
const LicensesComponent = () => {
    ...
    const [expiryDate, setExpiryDate] = useState(''); // YYYY-MM-DD format for input type="date"
    ...
    const fetchLicenses = async () => {
        ...
        try {
            ...
            setError('Error fetching licenses: ' + error.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const token = localStorage.getItem('token');
        const url = editingId
            ? `http://localhost:8000/api/licenses/${editingId}`
        ...
        try {
            ...
            if (response.ok) {
                ...
                setError(`Failed to save license: ${errorData.message || 'Unknown error'}`);
            }
        } catch (error) {
            setError('Error saving license: ' + error.message);
        }
    };
    ...
    const handleEdit = (license) => {
        ...
        setFile(null);
        if (license.path) {
            setPreview(`http://localhost:8000/${license.path}`);
        } else {
            setPreview(null);
        }
    };
    ...
    const handleDelete = async (id) => {
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`http://localhost:8000/api/licenses/${id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (response.ok) {
            ...
            setError('Error deleting license: ' + error.message);
        }
    };
        ...
        <Container maxWidth="lg">
            <Paper elevation={3} sx={{ p: 4, mt: 4 }}>
                <Typography variant="h4" component="h1" gutterBottom>
                    {editingId ? 'Редактировать лицензию' : 'Добавить лицензию'}
                </Typography>
                    ...
                    <Grid container spacing={3}>
                        ...
                        <Grid item xs={12}>
                            <Button type="submit" fullWidth variant="contained" startIcon={editingId ? <Edit /> : <Add />} sx={{ mt: 2, height: '56px' }}>
                                {editingId ? 'Обновить лицензию' : 'Добавить лицензию'}
                            </Button>
                        </Grid>
                        {editingId && (
                            <Grid item xs={12}>
                    ...
                    <List>
                        {licenses.map((license) => (
                            <ListItem
                                ...
                                secondaryAction={
                                    <>
                                        <IconButton edge="end" aria-label="edit" onClick={() => handleEdit(license)} sx={{ mr: 1 }}>
                                            <Edit />
                                        </IconButton>
                                        <IconButton edge="end" aria-label="delete" onClick={() => handleDelete(license.id)}>
                                            <Delete />
                                        </IconButton>
                                    </>
                                }
                            ...
                            >
                                <ListItemText
                                    primary={license.licenseName}
                                    secondary={
                                        <>
                                            <Typography component="span" variant="body2" color="text.primary">
                                                {license.expiryDate ? `Истекает: ${new Date(license.expiryDate).toLocaleDateString()}` : 'Дата истечения не указана'}
                                            </Typography>
                                            <br />
                                            {license.path ? (
                                                <Link href={`http://localhost:8000/${license.path}`} target="_blank" rel="noopener">
                                                    Скачать файл
                                                </Link>
                                            ) : 'Файл не загружен'}
                                        </>
